...
async def start(event):
  await event.reply("**🌀KarabakhtaggerBot**\n ile Grubunuzdakı Nerdeyse Tüm Üyelere Etiket Ata bilirim \nKomutlar için =======> /help yazın**",
		      ...
		      [Button.url('Beni Gruba Ekle ➕', 'https://t.me/KarabakhTaggerbot?startgroup=a')],
...
async def help(event):
  ...
  await event.reply(helptext,
                    buttons=(
                      [Button.url('Məni Qruba At➕', 'https://t.me/Karabakhggerbot?startgroup=a')],
                      [Button.url('Support👨‍💻', 'https://t.me/KarabakhTeamm')],
                      [Button.url('Resmi Kanal🔖', 'https://t.me/RiyaddBlog')],
		      [Button.url('Developer🧑‍🔧', 'https://t.me/Thagiyevvvv')],
                    ),
...
async def help(event):
  helptext = "**Çok özellikleri Etiket Botu Bulmaya Çalışan Grub Sahibleri @KarabakhTaggerBot Size Göre:\n\n📌 5-li etiket\n📌 Emoji etiket\n📌 Tekli Etiket\n📌 Yalnız Yöneticileri etiketleme\n📌\n\n Böyle Çok özellikli @LuciTaggerBot 'u grubunuza yönetici olarak ekleyip rahatlıkla üyelir , etiket ata bilirsiz **"
  await event.reply(helptext,
                    buttons=(
                      [Button.url('Botu Qruba At➕', 'https://t.me/KarabakhTaggerbot?startgroup=a')],
                    ),
                    link_preview=False
                   )
  ...
  if mode == "text_on_cmd":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      if event.chat_id not in tekli_calisan:
        await event.respond("**Işlem Başarıyla Durduruldu\n\n**Buda sizin reklamınız ola bilir @RiyadsBlog**❌****")
        return
      if usrnum == 1:
